...
class KrakenTests(unittest.TestCase):
    ...
    def kraken_run(self, method, plan, **kwargs):
        kwargs['with_tqdm'] = kwargs.get('with_tqdm', False)
        kwargs['pandas_extractor'] = kwargs.get('pandas_extractor', None)
        return kraken.Kraken.release(method, plan, **kwargs)
    ...
    def test_invoke(self):
        models = Query.combinatorics.grid(ctor=[Lasso, Ridge], alpha=[1, 2]).select(kraken.Kraken.invoke).to_list()
    ...
    def test_files(self):
        ...
        loaded_results = Query.en(kraken.Kraken.load(folder, None)).order_by(lambda z: z.result).to_list()
