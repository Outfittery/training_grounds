...
import pandas as pd
import inspect
from tg.common.datasets.local import *
...
class Preparer:
    ...
    def at(self, name, delta, is_major):
        self.time_table.append(PartitionedDatasetRecord(
            name,
    ...
    def ready(self):
        if len(self.time_table) > 0:
            PartitionedDatasetRecord.write_as_parquet(self.time_table, self.folder / 'description.parquet')
        return self.folder
...
class LocalTestCase(TestCase):
    def test_dataset_reading(self):
        ...
        ds = Dataset(path.parent, path.name, None)
    ...
    def test_partioned_dataset(self):
        ...
        ds = PartitionedDataset(path.parent, path.name, 'x')
        self.assertListEqual([8, 9, 6, 7, 0, 2, 4], list(ds.read().cnt))
        self.assertListEqual([2, 10, 7, 9, 1, 3, 8], list(ds.read().index))
        self.assertListEqual([2, 10, 7, 9], list(ds.read(count=4).index))

        df = ds.read(from_timestamp=DT(1))
        self.assertListEqual([8, 9, 6, 7], list(df.cnt))

        df = ds.read(from_timestamp=DT(1), to_timestamp=DT(3))
    ...
    def test_double_major(self):
        ...
        ds = PartitionedDataset(path.parent, path.name, 'x')
        self.assertListEqual([8, 9, 5, 6], list(ds.read().cnt))

        df = ds.read(from_timestamp=DT(2))
        self.assertListEqual([8, 9, 5, 6], list(df.cnt))

        df = ds.read(to_timestamp=DT(3))
